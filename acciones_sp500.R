...
consumer_discretionary_stock_arima_ts <-  ts(consumer_discretionary_stock_arima$consumer_discretionary_industry,start = 2015, end = 2023, frequency = 365)
ts_plot(consumer_discretionary_stock_arima_ts, 
        Xtitle = "Fecha",
        Ytitle = "Valor Acciones", 
        title ="Valor Consumer Discretionary Industry (2015-2023)", 
        slider = TRUE)
...
#Segunda diferencia
dd_log_consumer_discretionary_stock_arima_ts<-diff(d_log_consumer_discretionary_stock_arima_ts)
adf.test(dd_log_consumer_discretionary_stock_arima_ts)


#Tercera diferencia
ddd_log_consumer_discretionary_stock_arima_ts<-diff(dd_log_consumer_discretionary_stock_arima_ts)
adf.test(ddd_log_consumer_discretionary_stock_arima_ts)

#Correlograma Energy Stock
par(mfrow=c(1,2)) 
FAC_consumer_discretionary <- acf(d_log_consumer_discretionary_stock_arima_ts, main = "FAC - Consumer Discretionary Industry")# Correlograma FAC
...
ts_data_consumer_discretionary <- ts(log_consumer_discretionary_stock_arima_ts, start = c(2015,1), frequency = 365)
arima_consumer_discretionary_1 <- Arima(ts_data_consumer_discretionary, order = c(1,0,0))#, seasonal = list(order = c(1,1,2), period = 12))
...
plot(error_consumer_discretionary_1, main = "Residuos ARIMA(2,1,2) - Consumer Discretionary Industry",  xlab="Años",
     ylab="Error Model 1 - Consumer Discretionary Industry") #NO FUNCCIONA, PREGUNTAR PABLO

par(mar=c(4,4,2,1)) # ajustar los márgenes
plot(error_consumer_discretionary_1, main = "Residuos ARIMA(2,1,2) - Consumer Discretionary Industry", xlab="Años",
     ylab="Error Model 1 - Consumer Discretionary Industry") # trazar la gráfica
...
#Shapiro Wilk Test 
shapiro.test(error_consumer_discretionary_1)

#Análisis de Residuos 

checkresiduals(arima_consumer_discretionary_1)
...
consumer_discretionary_arima_forecast <-  forecast(arima_consumer_discretionary_1, h = 50, level=c(99.5))
autoplot(consumer_discretionary_arima_forecast) +
  autolayer(ts_data_consumer_discretionary, series = "Valores reales") +
  xlab("Año") +
  ylab("Valor") +
  ggtitle("Comparación de valores reales y predichos por ARIMA(2,0,1)(2,0,1)")
...
ts_plot(consumer_staples_stock_arima_ts, 
        Xtitle = "Fecha",
        Ytitle = "Valor Acciones", 
        title ="Valor Consumer Staples Industry (2015-2023)", 
        slider = TRUE)
...
#Identificación del modelo
auto.arima(log_consumer_staples_stock_arima_ts) #PREGUNTAR LEÓN SI ESTO SE DEBE HACER CON DF NORMAL O LOG
...
#Segunda diferencia
dd_log_consumer_staples_stock_arima_ts<-diff(d_log_consumer_staples_stock_arima_ts)
adf.test(dd_log_consumer_staples_stock_arima_ts)


#Tercera diferencia
ddd_log_consumer_staples_stock_arima_ts<-diff(dd_log_consumer_staples_stock_arima_ts)
adf.test(ddd_log_consumer_staples_stock_arima_ts)
...
arima_consumer_staples_1 <- Arima(log_consumer_staples_stock_arima_ts, order = c(2,1,2), seasonal = list(order = c(2,1,2), period = 12))
...
error_consumer_staples_1 <-  residuals(arima_consumer_staples_1)
mean(error_consumer_staples_1)
plot(error_consumer_staples_1, main = "Residuos ARIMA(2,1,2) - Consumer Staples Industry",  xlab="Años",
     ylab="Error Model 1 - Consumer Staples Industry") #NO FUNCCIONA, PREGUNTAR PABLO

par(mar=c(4,4,2,1)) # ajustar los márgenes
plot(error_consumer_staples_1, main = "Residuos ARIMA(2,1,2) - Consumer Staples Industry", xlab="Años",
     ylab="Error Model 1 - Consumer Staples Industry") # trazar la gráfica
...
consumer_staples_arima_forecast <-  forecast(arima_consumer_staples_1, h = 50, level=c(99.5))
...
#Segunda diferencia
dd_log_health_care_stock_arima_ts<-diff(d_log_health_care_stock_arima_ts)
adf.test(dd_log_health_care_stock_arima_ts)


#Tercera diferencia
ddd_log_health_care_stock_arima_ts<-diff(dd_log_health_care_stock_arima_ts)
adf.test(ddd_log_health_care_stock_arima_ts)
...
arima_health_care_1 <- Arima(ts_data_health_care, order = c(1,1,1), seasonal = list(order = c(1,1,1), period = 12))
...
#Shapiro Wilk Test 
shapiro.test(error_health_care_1)

#Análisis de Residuos 

checkresiduals(arima_health_care_1)
...
#Segunda diferencia
dd_log_industrials_stock_arima_ts<-diff(d_log_industrials_stock_arima_ts)
adf.test(dd_log_industrials_stock_arima_ts)


#Tercera diferencia
ddd_log_industrials_stock_arima_ts<-diff(dd_log_industrials_stock_arima_ts)
adf.test(ddd_log_industrials_stock_arima_ts)
...
industrials_energy_1 <- Arima(ts_data_industrials, order = c(2,1,2), seasonal = list(order = c(2,1,2), period = 12))
...
#Shapiro Wilk Test 
shapiro.test(error_industrials_1)

#Análisis de Residuos 

checkresiduals(arima_industrials_1)
...
#Segunda diferencia
dd_log_information_technology_stock_arima_ts<-diff(d_log_information_technology_stock_arima_ts)
adf.test(dd_log_information_technology_stock_arima_ts)


#Tercera diferencia
ddd_log_information_technology_stock_arima_ts<-diff(dd_log_information_technology_stock_arima_ts)
adf.test(ddd_log_information_technology_stock_arima_ts)
...
arima_information_technology_1 <- Arima(ts_data_information_technology, order = c(2,1,2), seasonal = list(order = c(2,1,2), period = 12))
...
plot(error_information_technology_1, main = "Residuos ARIMA(2,1,2) - Information Technology Industry",  xlab="Años",
     ylab="Error Model 1 - Information Technology Industry") #NO FUNCCIONA, PREGUNTAR PABLO

par(mar=c(4,4,2,1)) # ajustar los márgenes
plot(error_information_technology_1, main = "Residuos ARIMA(2,1,2) - Information Technology Industry", xlab="Años",
     ylab="Error Model 1 - Information Technology Industry") # trazar la gráfica
...
#Shapiro Wilk Test 
shapiro.test(error_information_technology_1)

#Análisis de Residuos 

checkresiduals(arima_information_technology_1)
...
materials_stock_arima_ts <-  ts(materials_stock_arima$energy_industry,start = 2015, end = 2023, frequency = 365)
...
media_materials_stock_arima <- mean(materials_stock_arima_ts)
...
#Identificación del modelo
auto.arima(log_materials_stock_arima_ts) #PREGUNTAR LEÓN SI ESTO SE DEBE HACER CON DF NORMAL O LOG
...
plot(error_materials_1, main = "Residuos ARIMA(2,1,2) - Materials Industry",  xlab="Años",
     ylab="Error Model 1 - Materials Industry") #NO FUNCCIONA, PREGUNTAR PABLO

par(mar=c(4,4,2,1)) # ajustar los márgenes
plot(error_materials_1, main = "Residuos ARIMA(2,1,2) - Materials Industry", xlab="Años",
     ylab="Error Model 1 - Materials Industry") # trazar la gráfica
...
#Shapiro Wilk Test 
shapiro.test(error_materials_1)

#Análisis de Residuos 

checkresiduals(arima_materials_1)
...
autoplot(forecast(arima_materials_1))
...
media_energy_stock_arima <- mean(energy_stock_arima_ts)
...
#Identificación del modelo
auto.arima(log_energy_stock_arima_ts) #PREGUNTAR LEÓN SI ESTO SE DEBE HACER CON DF NORMAL O LOG
...
#Segunda diferencia
dd_log_energy_stock_arima_ts<-diff(d_log_energy_stock_arima_ts)
adf.test(dd_log_energy_stock_arima_ts)


#Tercera diferencia
ddd_log_energy_stock_arima_ts<-diff(dd_log_energy_stock_arima_ts)
adf.test(ddd_log_energy_stock_arima_ts)

#Correlograma Energy Stock
par(mfrow=c(1,2)) 
FAC_energy <- acf(d_log_energy_stock_arima_ts, main = "FAC - Energy Industry")# Correlograma FAC
FACP_energy<- pacf(log_energy_stock_arima_ts, main = "FACP - Energy Industry") #Correlograma FACP

#FASE DE ESTIMACIÓN

#Analizando la FAC y la FACP
#arima_energy_1<-arima(d_log_energy_stock_arima_ts, order=c(2,3,1)) 
ts_data <- ts(log_energy_stock_arima_ts, start = c(2015,1), frequency = 365)
arima_energy_1 <- Arima(ts_data, order = c(2,1,2), seasonal = list(order = c(2,1,2), period = 12))
...
mean(error_energy_1)
plot(error_energy_1, main = "Residuos ARIMA(2,1,2) - Energy Industry",  xlab="Años",
     ylab="Error Model 1 - Energy Industry") #NO FUNCCIONA, PREGUNTAR PABLO

par(mar=c(4,4,2,1)) # ajustar los márgenes
plot(error_energy_1, main = "Residuos ARIMA(2,1,2) - Energy Industry", xlab="Años",
     ylab="Error Model 1 - Energy Industry") # trazar la gráfica
...
#Shapiro Wilk Test 
shapiro.test(error_energy_1)

#Análisis de Residuos 

checkresiduals(arima_energy_1)
...
autoplot(energy_arima_forecast) +
  autolayer(ts_data, series = "Valores reales") +
  xlab("Año") +
  ylab("Valor") +
  ggtitle("Comparación de valores reales y predichos por ARIMA(2,0,1)(2,0,1)[350]")
...
autoplot(forecast(arima_energy_1))
...
#https://cran.rstudio.com/web/packages/TSstudio/vignettes/Plotting_Time_Series.html
...
#El p-value () es mayor que el nivel de significación (0.05), por tanto, no podemos 
# rechazar la hipótesis nula, es decir, los errores no están correlacionados

#Los errores presentan una media cercana a cero () y no están correlacionados
# por lo que podemos dar como válido el modelo ARIMA()
...
#Heterocedasticidad Eneergy Industry
#heter_energy_1<- arch.test(arima_energy_1) #ARIMA(2,1,2)
...
#arima_energy_1<-arima(d_log_energy_stock_arima_ts, order=c(2,1,1)) 

#forecast_energy_arima <- autoplot(forecast(arima_energy_1))
#print(forecast_energy_arima)
...
#ts_data <- ts(log_energy_stock_arima_ts, start = c(2015,1), frequency = 365)
#arima_energy_1 <- Arima(ts_data, order = c(2,1,1), seasonal = list(order = c(2,1,1), period = 12))
...
#predict(arima_italy_1,n.ahead = 5)
...
# Regresión Lineal - Energy Industry --------------------------------------

# crear un vector de índices aleatorios
indices_energy <- sample(nrow(energy_stock_def), nrow(energy_stock_def)*0.7)
colnames(energy_industry)

# crear los conjuntos de entrenamiento y prueba
energy_train_rl <- energy_stock_def[indices_energy, ]
energy_test_rl <- energy_stock_def[-indices_energy, ]

energy_train_rl$energy_industry
colnames(energy_train_rl)
#Separación de datos


arbol_completo <-  rpart(energy_industry  ~ .  ,
                         data = energy_train_rl)
plot(arbol_completo)
text(arbol_completo)
rpart.plot::rpart.plot(arbol_completo)
...
lm_energy <- lm(energy_industry ~ . , data = energy_train_rl)
summary(lm_energy)
...
#Cálculo de R^2
summary(lm_energy)$r.sq

## Conclusiones que podemos extraer: 

rse_energy <- summary(lm_energy)$sigma #RSE 406228.1


# Contraste de residuos:
## Ho: normalidad de residuos
## H1: no hay normalidad de residuos

## Test: Shapiro-Wilk normality lm_modelo_1
residuos_energy_1 <- residuals(lm_energy)
shapiro.test(residuos_energy_1)


## Test: Kolmogorov-Smirnov
ks.test(residuos_energy_1, pnorm) 


#Test Sahpiro - wilk normality: houses_train
residuos_energy_train <-  residuals(lm_energy)
shapiro.test(residuos_energy_train)

#Contraste de residuos
#HO: Normalidad de los residuos
#HI: No hay normalidad
qqnorm(residuos_energy_1)
qqline(residuos_energy_1)

plot(fitted(lm_energy), residuos_energy_1)


#Contraste de residuos houses_train
#HO: Normalidad de los residuos
#HI: No hay normalidad
qqnorm(residuos_energy_train)
qqline(residuos_energy_train)

plot(fitted(lm_energy), residuos_energy_train)
houses_train$date <- as.Date(houses_train$date)



#Prediccion ---------------------
#VAmos a predecir los valores de price
prediccion_1 <-   predict(object = lm_energy, newdata = energy_test_rl)

prediccion_2 <-  predict(object = lm_modelo_1, newdata = houses_test)

sqrt(mean((prediccion_1 - energy_train_rl$energy_industry)**2)) #RMSE obtenido pred 406025

sqrt(mean((prediccion_2 - houses_train$price)**2)) #RMSE obtenido pred 236414.8


#Intervalo de confianza:    
predict(lm_houses_train, newdata = houses_train, interval="confidence", level = 0.95)    

#Predicción por intervalo
predict(object = lm_houses_train, newdata = houses_train, interval = "prediction")
...
#https://rstudio-pubs-static.s3.amazonaws.com/624513_b2906a1e85144e16bcdf15286afaa78a.html



# MODELO XGBOOST GITHUB (NO) ------------------------------------------------

#https://github.com/niki864/Simple-Stock-Predictor-xgboost-knn-
library(xgboost)
library(quantmod)
library(TTR)
library(highcharter)

highchart(type = "stock") %>% hc_add_series(energy_industry,type="ohlc") %>% 
  hc_add_series(real_estate_industry,type="ohlc") %>% hc_add_series(materials_industry, type="ohlc") %>%
  hc_add_series(information_technology_industry, type="ohlc") %>% hc_legend(enabled=TRUE) %>% hc_title(text="OHLC Prices For The Last 5 Years")

highchart(type = "stock") %>% 
  hc_add_series(sp500_industry_mean$utilities_industry, type = "line")

highchart(type = "stock") %>% 
  hc_add_series(data = sp500_industry_mean, type = "line", hcaes(x = Date, y = utilities_industry))

highchart() %>%
  hc_xAxis(categories = sp500_industry_mean$Date) %>% # Se establecen las fechas en el eje X
  hc_add_series(data = sp500_industry_mean, type = "line", hcaes(x = Date, y = utilities_industry),
                hcaes(x = Date, y = real_estate_industry), hcaes(x = Date, y = materials_industry),
                hcaes(x = Date, y = information_technology_industry), hcaes(x = Date, y = industrial_industry),
                hcaes(x = Date, y = health_care_industry), hcaes(x = Date, y = financial_industry),
                hcaes(x = Date, y = energy_industry), hcaes(x = Date, y = communication_service_industry),
                hcaes(x = Date, y = consumer_discretionary_industry), hcaes(x = Date, y = consumer_staples_industry),
                hcaes(x = Date, y = INVUS500),
                name = names(sp500_industry_mean))


#Relative StrengthnON Indicator

rsiFNCL <-RSI(energy_industry$energy_industry, n=14, maType = "WMA")
plot(rsiFNCL, type = 'l')
...
# Instalar y cargar la biblioteca neuralnet
#install.packages("neuralnet")
install.packages("caret")
#install.packages("h2o") #Paquete pesado, puede tener mucha utilidad
install.packages("tensorflow")
library(neuralnet)

#Normalizamos los daos


# Dividir los datos en conjuntos de entrenamiento y prueba LAS PRUEBAS SE VAN A HACER CON ENERGY INDUSTRY
energy_industry_train <- energy_stock_def[1:1900, ]
energy_industry_test <- energy_stock_def[1901:2027, ]

# Crear la red neuronal con una capa oculta y 10 neuronas
red_neuronal <- neuralnet(energy_industry ~ . , data = energy_industry_train, hidden = 10)

# Entrenar la red neuronal
entrenamiento <- compute(red_neuronal, energy_industry_train[, 1:3])

# Hacer predicciones en los datos de prueba
predicciones <- compute(red_neuronal, energy_industry_test[, 1:3])



# PRUEBAS ENERGY ----------------------------------------------------------

# Define el nombre del dataframe
df_name <- "energy_stock_def"

# Define la cantidad de veces que se ejecutará el bucle
num_iterations <- 10

# Define una función para cargar y modificar el dataframe
load_dataframe <- function() {
  ...
  energy_stock_def <- select(energy_stock_def, -utilities_industry, -real_estate_industry, 
                             ...
                             -financial_industry ,-communication_service_industry , 
                             -consumer_discretionary_industry, -INVUS500_mean, -consumer_staples_industry )
  return(energy_stock_def)
}

# Ejecuta el bucle
for (i in 1:num_iterations) {
  # Verifica si el dataframe ya existe y elimínalo si es necesario
  if (exists(df_name)) {
    rm(list = df_name)
  }
  
  # Carga el dataframe dentro de un bloque tryCatch
  tryCatch({
    energy_stock_def <- load_dataframe()
  }, error = function(e) {
    message("Error al cargar o modificar el dataframe: ", e$message)
  })
  ...
  if (exists(df_name)) {
    colnames(energy_stock_def)
    energy_industry <- subset(energy_stock_def, select = c("Date", "energy_industry"))
    message(paste("El dataframe", df_name, "se ha cargado correctamente."))
  } else {
    message("No se pudo cargar el dataframe.")
  }
}
